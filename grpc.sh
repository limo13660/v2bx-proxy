#!/usr/bin/env bash
# V2bX Nginx reverse proxy installer (gRPC + TLS)
# Purpose: keep original flow (domain -> cert -> nginx -> grpc/tls proxy -> output panel params)
...
getData() {
    ...
    colorEcho "$BLUE"   "  3. 面板 / V2bX 节点传输方式需要改为 gRPC，serviceName 要与本脚本一致"
    echo ""
    ...
    echo ""
    read -r -p " 请输入 Nginx 连接端口[100-65535，默认443]：" PORT
    [[ -z "$PORT" ]] && PORT=443
    ...
    while true; do
        ...
        if [[ -z "$GRPC_SERVICE" ]]; then
            local len svc
            len="$(shuf -i 5-12 -n 1)"
            svc="$(tr -dc 'a-zA-Z0-9' </dev/urandom | fold -w "$len" | head -n 1)"
            GRPC_SERVICE="$svc"
            break
...
showSummary() {
    echo ""
    success "安装完成，V2bX 服务端口为 ${V2PORT}，Nginx 连接端口为 ${PORT}!"
    success "----------- gRPC + TLS 关键参数 -----------------------------"
    colorEcho "$RED"   " 服务/后端监听端口：${V2PORT}"
    colorEcho "$RED"   " 用户连接端口：${PORT}"
    colorEcho "$RED"   " 域名(SNI)：${DOMAIN}"
    colorEcho "$RED"   " 传输协议：grpc"
    ...
    info "证书文件：${CERT_FILE}"
    info "请确认面板 / V2bX 节点已改为 network=grpc、security=tls、serviceName=${GRPC_SERVICE}。"
...
menu() {
    ...
    echo "================================"
    echo ""
    echo -e "  ${GREEN}1.${PLAIN}   为 V2bX 添加 Nginx 反代(gRPC+TLS)"
